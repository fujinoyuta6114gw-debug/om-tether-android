# OM Tether for Android — USB-C prototype v0.2.1
...
このソースは、Android側のUI・保存・画像解析とPTP通信層までを実装した**実機検証前のプロトタイプ**です。OM‑1 Mark IIのUSB制御はメーカー公開SDKではなく、カメラが公開するPTP能力とlibgphoto2の公開実装を根拠にしています。ファームウェアやAndroid端末のUSB実装差があるため、本番撮影に使う前に必ず予備カードとテスト撮影で確認してください。

| 項目 | 状態 |
|---|---|
| デモのライブビュー／ヒストグラム／白飛び表示 | 実装済み、ソース検証済み |
| JPEGのMediaStore保存 | 実装済み |
| USB列挙・権限・PTPバルク通信 | 実装済み、実機確認が必要 |
| OM‑1 Mark IIライブビュー | 実装済み、実機確認が必要 |
| JPEG／ORF自動取得 | 実装済み、カメラをRAW+JPEGに設定して実機確認が必要 |
| 露出値の変更 | カメラが列挙する値だけを送る安全側実装、表示名の実機調整が必要 |
...
1. OM‑1 Mark IIのUSB接続モードを **RAW/Control** にします。
2. カメラ側の画質を **RAW+JPEG** にします。アプリはカメラが生成した両方のオブジェクトを保存します。
3. 充電専用ではないUSB-CデータケーブルでAndroid端末へ直結します。
4. Android端末がUSBホスト（OTG）に対応し、カメラへ十分な給電ができることを確認します。長時間運用では給電対応USB-Cハブを推奨します。
5. アプリを起動し「USB接続」を押して、AndroidのUSBアクセス確認を許可します。
...
`.github/workflows/build-apk.yml` を同梱しています。GitHubの `main` ブランチへ配置すると、ソース検証、JVM単体テスト、debug APK生成を自動実行します。成功時の成果物名は `OM-Tether-v0.2.1-debug` で、中に `app-debug.apk` が入ります。

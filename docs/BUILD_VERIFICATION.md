...
- ContentScale.Fitに対する倍率から確認画像1px＝画面1pxとなる100％倍率を算出
- 正規化した確認中心を写真間で保持し、異なる縦横サイズの最新2枚でも同じ位置へ移動
- 中央、前回位置、任意タップ位置、端末内顔検出位置へのジャンプを追加
- Sobel勾配の分位しきい値によるフォーカスマスクと11段階の感度調整を追加
- マスク解析を長辺1024px、顔検出を長辺1024pxへ制限し、どちらもDefaultワーカーで実行
- JPEG本体またはORF内蔵プレビューから長辺最大3072pxのRGB_565確認画像を生成
- 高解像度確認画像を最新2枚だけに制限し、直近20枚の履歴サムネイルとは分離
- 100％倍率、表示中心のクランプ、任意位置の逆座標変換、マスクしきい値をJVM単体テストへ追加
- カメラ制御命令を追加しておらず、削除／フォーマット命令がないことを構造検証で再確認

v0.6.0はGitHub Actionsで構造検証、JVM単体テスト、APK生成後、JPEG／ORF各2枚で「100％・前回位置・中央・顔・任意位置・左右比較・マスク感度」を確認します。

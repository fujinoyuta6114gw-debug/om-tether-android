...
イベント通知の既知値は標準ObjectAdded `0x4002`、Olympus `0xC002`、OM‑1 Mark II系 `0xC102` です。この試作は割り込みINエンドポイントがある場合に3種類をすべて受け取り、撮影時には `GetObjectHandles` の差分も確認します。`GetObjectHandles` のstorage IDには `0xFFFFFFFF`（全ストレージ）を指定するため、カード1／カード2のどちらへ作成されたオブジェクトも候補になります。割り込み通知とカード上の差分のどちらも得られない場合は、`GetImage` を読み取り専用フォールバックにしています。

v0.3.2ではObjectAddedの監視をアプリ内シャッター待機時だけでなく接続中ずっと継続し、カメラ本体シャッターで作成されたハンドルも転送対象にしました。v0.3.3では、接続完了前の全ストレージ走査が一部の実機で応答待ちになるため廃止しました。USB割り込みのObjectAdded通知を優先し、割り込みエンドポイントがない、初期化できない、または途中終了した場合だけ約1.2秒ごとの全ストレージ差分監視へ切り替えます。ポーリング経路の最初の成功結果は既存カード内容の基準として扱い、新規撮影だけを取り込みます。RAW／JPEGがカード1／2へ時間差で追加される場合に備え、追加が静まるまで短時間まとめてから保存形式を選別します。

v0.3.4では割り込みエンドポイントが存在してもObjectAdded通知が流れない実機経路を考慮し、最初の正常なライブビューJPEG受信後に全ストレージ差分監視を常時併用します。ライブビューを長時間止めないよう、この監視のbulk転送には通常のPTP命令より短いタイムアウトを設定します。初回基準作成より先にObjectAddedを受信した場合は、そのイベントと数値的に近い少数のハンドルだけを候補に残し、ObjectInfoのファイル名 stem またはcapture dateが一致したRAW／JPEGだけを同一撮影として選別します。露出記述子の読み取りも最初のライブビュー表示後へ移し、各照会を短時間に制限してフレーム取得と交互に処理します。
...
v0.3.6では、Androidの`UsbEndpoint.getMaxPacketSize()`で得られるPTPバルクエンドポイントの最大パケットサイズを使い、USB 3.x相当（1024バイト）／USB 2.0相当（512バイト）を接続ガイドへ表示します。これはケーブル単体の速度認証ではなく、端末・アダプター・カメラを含む現在の列挙経路の推定です。v0.3.7では、推定対象を実際に使用するStill Image/PTPインターフェースへ限定し、診断ログにもIN/OUTの値を残します。

v0.3.7では、Androidの`UsbDeviceConnection.requestWait(timeout)`が返す`null`をイベント待ちの正常なタイムアウトとして扱います。イベントがないだけで割り込み監視を終了せず、カード1／カード2の差分ポーリングと併用して本体シャッターの取り込みを継続します。USB再接続時はキャンセルされたサスペンド処理を古いコントローラー上で続行しないよう、キャンセル例外をUSB初期化・露出照会・ライブビュー再開の境界で再送出します。
